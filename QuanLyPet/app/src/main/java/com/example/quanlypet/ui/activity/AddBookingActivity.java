    ...
    public class AddBookingActivity extends AppCompatActivity {
        ...
        private void addBooking() {
            ...
            if (rdoPhongkham.isChecked()) {
                ...
                Toast.makeText(this, "Vui lòng chọn nơi khám.", Toast.LENGTH_SHORT).show();
                return;
            }

            // Kiểm tra thông tin bắt buộc (ngoại trừ địa chỉ nếu là phòng khám)
            if (strTT.isEmpty() || strTime.isEmpty() || strTimeHold.isEmpty() || strDichVU.isEmpty()) {
                Toast.makeText(this, "Vui lòng nhập đầy đủ thông tin trước khi đặt lịch.", Toast.LENGTH_SHORT).show();
                return;
            }

            // Nếu là khám tại nhà thì địa chỉ bắt buộc
            if (noikham.equals("Tại nhà") && strDiaChi.isEmpty()) {
                Toast.makeText(this, "Vui lòng nhập địa chỉ khi chọn khám tại nhà.", Toast.LENGTH_SHORT).show();
                return;
            }
...
//            if (checkBooking() == 0 && checkDoctorConflict() == 0) {
//                // Không trùng lịch tổng quát và cũng không trùng bác sĩ
//                BookObj bookObj = new BookObj(id, idDoctor, idPet, strTT, anh, strTime, strTimeHold, noikham, strDiaChi, strDichVU, 1);
//                BookDB.getInstance(this).Dao().insert(bookObj);
//                sendNotification();
//                showSnackbar();
...
//                }, 3000);
//            } else {
//                showSnackbar2(); // Có xung đột lịch
//            }
        }
public void SlectedSpinner() {
    ...
    spnPet.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
        @Override
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
            // Sử dụng cùng SharedPreferences key với phần khác
            SharedPreferences sharedPreferences = getSharedPreferences("user_file", MODE_PRIVATE);
            String username = sharedPreferences.getString("Username", "");
            usersObj = UsersDB.getInstance(getApplicationContext()).Dao().getIdUsers(username);

            // Lấy danh sách thú cưng của người dùng hiện tại
            listAnimal = AnimalDB.getInstance(getApplicationContext()).Dao().getIDUsers(String.valueOf(usersObj.getId()));
        ...
        //check bac si moi them///////////
        public int checkDoctorConflict() {
            String time = TIEDTime.getText().toString();
            String timeHold = TIEDTimeHold.getText().toString();

            List<BookObj> conflictList = BookDB.getInstance(this).Dao()
                    .checkDoctorBookingConflict(time, timeHold, idDoctor);

            return conflictList.isEmpty() ? 0 : 1;
        }
        // moi them


        public int checkBooking() {
            String startTime = TIEDTime.getText().toString();
            String endTime = TIEDTimeHold.getText().toString();

            List<BookObj> conflictList = BookDB.getInstance(this).Dao()
                    .checkDoctorBookingConflict(startTime, endTime, idDoctor);
...
public void showSnackbar2() {
    ...
    // Cập nhật nội dung động vào TextView
    TextView detailText = custom.findViewById(R.id.title_thongbaochitiet);
    detailText.setText("Khung giờ từ " + TIEDTime.getText().toString()  + " đến " + TIEDTimeHold.getText().toString()+ " đã được đặt. Vui lòng chọn thời gian khác.");

    // Tạo snackbar rỗng để chứa layout tùy chỉnh
    Snackbar snackbar = Snackbar.make(btnBooking, "", Snackbar.LENGTH_LONG);
    snackbar.getView().setBackgroundColor(Color.TRANSPARENT);

    // Thêm layout tùy chỉnh vào snackbar
    Snackbar.SnackbarLayout snackbarLayout = (Snackbar.SnackbarLayout) snackbar.getView();
    snackbarLayout.setPadding(0, 0, 0, 0);
    snackbarLayout.addView(custom, 0);
